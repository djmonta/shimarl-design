...
import { FaCaretRight, FaCaretLeft, FaClose } from 'react-icons/fa'
...
class GatsbyGramModal extends React.Component {
  ...
  findCurrentIndex() {
    ...
    index = findIndex(
      posts,
      post => post.slug === this.props.location.pathname.split(`/`)[1]
    )
  ...
  render() {
    return (
      <StaticQuery
        ...
        render={data => {
          if (!posts) {
            posts = data.allMarkdownRemark.edges.map(e => e.node)
          }
          return (
            <Modal
